...
export function WaterfallContent({
  ...
  function renderTooltipContent(payload: { payload?: { formula: string; label: string; displayValue: number; id: string } }[]) {
    ...
    const formula = isShipping && breakdown
      ? `Forward charges (${formatCurrency(breakdown.forwardCharges, currency)}) + COD charges (${formatCurrency(breakdown.codCharges, currency)}). Same source as Logistics.`
      : p.formula
    return (
      <div className="border-border bg-background grid min-w-[16rem] gap-1.5 rounded-lg border px-3 py-2 text-xs shadow-lg">
        ...
        <div className="pt-1 font-mono tabular-nums">
          {p.displayValue >= 0
            ? formatCurrency(p.displayValue, currency)
            : `-${formatCurrency(Math.abs(p.displayValue), currency)}`}
        </div>
      </div>
    )
  }
                ...
                <ChartContainer config={chartConfig} className="h-full w-full">
                  ...
                  >
                    <XAxis
                      type="number"
                      tickFormatter={(v) =>
                        formatCurrency(v, currency, { notation: 'compact', compactDisplay: 'short' })
                      }
                      tick={{ fontSize: 11 }}
                    />
                    <YAxis
